...
def tok_map(sample):
    return tokenizer(sample["sentence1"], sample["sentence2"], truncation = True, padding = "max_length", max_length = 512)
...
ini_learning_rate = [0.0001] #[1e-5, 5e-5, 1e-4]
n_epochs = [10] #[5, 7, 10]
...
for n_ep, ini_lr in product(n_epochs, ini_learning_rate):
    ...
    output_dir = rf"./FunTrf-PAWS_lr-{ini_lr}_nep-{n_ep}"
    batch_size = 32
    ...
    with open(rf"./FunTrf-PAWS_lr-{ini_lr}_nep-{n_ep}_train.csv", "w", encoding = "utf-8") as f:
    ...
    with open(rf"./FunTrf-PAWS_lr-{ini_lr}_nep-{n_ep}_val.csv", "w", encoding = "utf-8") as f:
    ...
    with open(rf"./FunTrf-PAWS_hyperparamter_evaluation.csv", "a", encoding = "utf-8") as f:
        f.write(cad_scores)
