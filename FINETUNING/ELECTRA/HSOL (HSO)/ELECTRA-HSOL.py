...
for n_ep, ini_lr in product(n_epochs, ini_learning_rate):
    ...
    # Model: ELECTRA with a classification head of 3 classes
    ...
    output_dir = rf"./ELECTRA-HSOL_lr-{ini_lr}_nep-{n_ep}"
    batch_size = 32
    ...
    with open(rf"./ELECTRA-HSOL_lr-{ini_lr}_nep-{n_ep}_train.csv", "w", encoding = "utf-8") as f:
    ...
    with open(rf"./ELECTRA-HSOL_lr-{ini_lr}_nep-{n_ep}_val.csv", "w", encoding = "utf-8") as f:
    ...
    with open(rf"./ELECTRA-HSOL_hyperparameter_evaluation.csv", "a", encoding = "utf-8") as f:
        f.write(cad_scores)
