...
def tok_map(sample):
    return tokenizer(sample["premise"], sample["hypothesis"], truncation = True, padding = "max_length")#, max_length = 96)
...
for n_ep, ini_lr in product(n_epochs, ini_learning_rate):
    ...
    # Model: DistilBERT with a classification head of 3 classes
    ...
    output_dir = rf"./DistilBERT-NLI_lr-{ini_lr}_nep-{n_ep}"
    batch_size = 32
    ...
    with open(rf"./DistilBERT-NLI_lr-{ini_lr}_nep-{n_ep}_train.csv", "w", encoding = "utf-8") as f:
    ...
    with open(rf"./DistilBERT-NLI_lr-{ini_lr}_nep-{n_ep}_val.csv", "w", encoding = "utf-8") as f:
    ...
    with open(rf"./DistilBERT-NLI_hyperparameter_evaluation.csv", "a", encoding = "utf-8") as f:
        f.write(cad_scores)
