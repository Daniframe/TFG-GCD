...
for n_ep, ini_lr in product(n_epochs, ini_learning_rate):
    ...
    output_dir = rf"./DistilBERT-GC_lr-{ini_lr}_nep-{n_ep}"
    batch_size = 32
    ...
    with open(rf"./DistilBERT-GC_lr-{ini_lr}_nep-{n_ep}_train.csv", "w", encoding = "utf-8") as f:
    ...
    with open(rf"./DistilBERT-GC_lr-{ini_lr}_nep-{n_ep}_val.csv", "w", encoding = "utf-8") as f:
